...
def _retry():
    ...
    return _CappedRetry(
        ...
        # Obey a server that asks us to wait, but never for an hour: the cap
        # is what keeps one rude host from freezing a whole run. 429 is
        # deliberately absent above -- the governor's cooldown answers that,
        # and a urllib3 retry would only spend more of the budget.
